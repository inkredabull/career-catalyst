...
export function getLinkedinURL(filter: SearchFilter): string {
  ...
  const decorationId =
    "com.linkedin.voyager.dash.deco.jobs.search.JobSearchCardsCollection-216";
...
export function extractInfo(
  ...
  if (Array.isArray(included)) {
    ...
    included.forEach((raw: unknown) => {
      ...
      const applicants = extractApplicantCount(
        item.jobInsightsV2ResolutionResults,
      );
      if (applicants !== undefined && applicants > 200) {
        log(
          "DEBUG",
          "Excluded (>200 applicants: %s): [%s] %s",
          applicants,
          search,
          title,
        );
      ...
      const company =
        (item.primaryDescription as { text?: string } | undefined)?.text ?? "";
      const location = (
        item.secondaryDescription as { text?: string } | undefined
      )?.text;
      const info = (item.tertiaryDescription as { text?: string } | undefined)
        ?.text;
    ...
    log(
      "DEBUG",
      "[%s] %s included → %s job cards → %s titled → %s passed patterns → %s passed applicant filter → %s kept",
      search,
      included.length,
      jobCards,
      jobCards - noTitle,
      jobCards - noTitle - patternFiltered,
      jobCards - noTitle - patternFiltered - applicantFiltered,
      kept,
    );
  }
...
export async function getSearchResultsFromLinkedin(
  filter: SearchFilter,
...
const TOP_APPLICANT_REFERER =
  "https://www.linkedin.com/jobs/collections/top-applicant/";
const TOP_APPLICANT_PAGES = 2;

export async function getTopApplicantFromLinkedin(): Promise<
...
> {
  ...
  for (let page = 0; page < TOP_APPLICANT_PAGES; page++) {
    ...
    log(
      "DEBUG",
      "LinkedIn Top Applicant HTTP %s (page %s)",
      response.status,
      page + 1,
    );
    if (!response.ok) {
      log(
        "WARN",
        "LinkedIn Top Applicant HTTP %s (page %s): %s",
        response.status,
        page + 1,
        text.slice(0, 300),
      );
