...
describe('extractApplicantCount', () => {
  ...
  it('returns undefined for "Be an early applicant"', () => {
    expect(extractApplicantCount([{ text: 'Be an early applicant' }])).toBeUndefined();
  });
  ...
  it('finds the count nested inside a complex object', () => {
    const insights = [{ insightViewModel: { text: '87 applicants', accessibilityText: '87 applicants' } }];
